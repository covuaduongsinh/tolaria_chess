...
- The "Import chess games" command palette action opens `ChessImportDialog`. Pasted PGN flows through pure helpers — `pgnImport.ts` (split a multi-game file, read headers incl. `Opening`/`Variation`), `chessEntities.ts` (extract/normalize/dedup the distinct players, openings, and events and resolve them against existing notes), `chessTypeSeeds.ts` (the four chess type documents), `chessGameNote.ts` (build typed `Chess Game` frontmatter plus an embedded board), and `chessGameImport.ts` (orders the writes type → entity → game with collision-free names and an injected `createNote`) — and each note is written via `create_note_content`. Each import joins the **knowledge graph** (ADR-0141): `chess_white`/`chess_black`/`chess_event`/`chess_opening` are emitted as `[[wikilink]]` relationships pointing at idempotently auto-created `Chess Player`/`Chess Opening`/`Chess Event` stub notes (reusing any matching note already in the vault), while `chess_result`/`chess_date`/`chess_eco` stay scalar for filtering. Games, players, openings, and events are therefore browsable through sidebar type grouping, backlinks, Neighborhood, and search — not just `chess_*` property filters.
